...
public class PaymentController {
    ...
    public ResponseEntity<String> handlePaymentRequest(@PathVariable("customerId") String customerId) {
    ...
    @GetMapping("/registrazione/{userId}")
    public ResponseEntity<String> getUserDetails(@PathVariable("userId") Long userId) {
	...
	try {
		...
		return new ResponseEntity<>("Stripe customer ID not found for the user", HttpStatus.NOT_FOUND);
	    }
	} catch (StripeException e) {
	    e.printStackTrace();
	    return new ResponseEntity<>("Error retrieving user details. Please try again later.",
		    HttpStatus.INTERNAL_SERVER_ERROR);
	}
    }
